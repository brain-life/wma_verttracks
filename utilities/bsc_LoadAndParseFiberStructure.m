...
if ischar(feORwbfg)
    % loads if a path
    %if a .tck path is detected, use relevant function
    if strcmp(feORwbfg(end-3:end),'.tck')
        wbFG =  dtiImportFibersMrtrix(feORwbfg, .5);
        fe=[];
    else
    %otherwise, use standard mba/vistasoft functions
        load(feORwbfg)
...
elseif isstruct(feORwbfg)
    %FUNCTION HAS NO WAY TO HANDLE .TCK OBJECTS CURRENTLY.  FIGURE THIS OUT.
